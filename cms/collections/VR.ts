...
export const VRScenes: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "title",
    defaultColumns: ["title", "museum", "sceneOrder", "isActive"],
    group: "Places",
  },
...
export const VRHotspots: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "title",
    defaultColumns: ["title", "scene", "type", "isActive"],
    group: "Places",
  },
  access: publicReadEditorWrite,
  fields: [
    ...
    { name: "description", type: "textarea", localized: true },
    {
      ...
      required: true,
      defaultValue: 50,
      admin: { description: "X position (0-100)" },
    },
    {
      name: "positionY",
      type: "number",
      required: true,
      defaultValue: 50,
      admin: { description: "Y position (0-100)" },
    },
    {
      name: "content",
      type: "json",
      admin: {
        description:
          "Type-specific content (duration, audioUrl, readTime, etc.)",
      },
    },
    {
      name: "targetScene",
      type: "relationship",
      relationTo: "vr-scenes",
      admin: { condition: (d) => d?.type === "next-scene" },
    },
    {
      name: "isActive",
      type: "checkbox",
