...
export const ContentAccess: CollectionConfig = {
  ...
  admin: {
    ...
    defaultColumns: ["contentType", "contentId", "tier", "sponsor"],
    group: "Access",
  },
  access: publicReadAdminWrite,
  fields: [
    ...
    },
    {
      name: "priceCents",
      type: "number",
      admin: { description: "Individual purchase price in cents" },
    },
...
export const TourAgencies: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "name",
    defaultColumns: ["name", "contactEmail", "verificationStatus", "isActive"],
    group: "Access",
  },
  access: {
    read: publicRead,
...
export const AgencyAccessCodes: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "code",
    defaultColumns: ["code", "agency", "usesCount", "maxUses", "isActive"],
    group: "Access",
  },
  access: {
    read: ({ req: { user } }) => {
      if (user?.role === "admin") return true;
      if (isAgencyUser(user) && user?.id)
        return { agency: { adminUser: { equals: String(user.id) } } } as any;
      return false;
    },
    create: ({ req: { user } }) => user?.role === "admin" || isAgencyUser(user),
    update: ({ req: { user } }) => user?.role === "admin" || isAgencyUser(user),
    delete: isAdmin,
  },
  fields: [
    {
    ...
    { name: "code", type: "text", required: true, unique: true },
    {
      name: "name",
      type: "text",
      admin: { description: 'e.g. "July 2026 Group"' },
    },
...
export const UserContentAccess: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "accessType",
    defaultColumns: ["user", "accessType", "contentType", "expiresAt"],
    group: "Access",
  },
  access: ownerAccess,
  fields: [
    ...
    },
    {
      name: "contentType",
      type: "text",
      admin: { description: "null for subscriptions (access all)" },
    },
...
export const CodeRedemptions: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "id",
    defaultColumns: ["code", "user", "redeemedAt"],
    group: "Access",
  },
  access: ownerAccess,
  fields: [
    ...
    { name: "user", type: "relationship", relationTo: "users", required: true },
    {
      name: "access",
      type: "relationship",
      relationTo: "user-content-access",
      required: true,
    },
...
export const AgencyPricingPlans: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "name",
    defaultColumns: ["name", "planType", "priceCents", "isActive"],
    group: "Access",
  },
  access: publicReadAdminWrite,
  hooks: { beforeValidate: [autoSlug] },
  fields: [
    ...
    },
    {
      name: "includedCodes",
      type: "number",
      admin: { description: "null for unlimited" },
    },
...
export const AgencyPurchases: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "id",
    defaultColumns: ["agency", "plan", "status", "purchasedAt"],
    group: "Access",
  },
  access: {
    read: ({ req: { user } }) => {
  ...
  fields: [
    ...
    },
    {
      name: "plan",
      type: "relationship",
      relationTo: "agency-pricing-plans",
      required: true,
    },
...
export const Payments: CollectionConfig = {
  ...
  admin: {
    useAsTitle: "id",
    defaultColumns: ["user", "amount", "currency", "gateway", "status"],
    group: "Access",
  },
  access: ownerAccess,
  fields: [
